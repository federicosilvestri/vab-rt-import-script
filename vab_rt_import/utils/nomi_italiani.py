...
_FILE_NAME = Path(__file__).parent / 'data' / 'italian_names.txt'
...
def process_names():
    ...
    with codecs.open(str(_FILE_NAME.absolute()), 'w', encoding='utf-8') as file:
...
def load_names():
    ...
    if not _FILE_NAME.exists():
        raise RuntimeError(f'File {_FILE_NAME} not found. You must run nomi_italiani.py first.')
    with codecs.open(str(_FILE_NAME.absolute()), 'r', encoding='utf-8') as file:
